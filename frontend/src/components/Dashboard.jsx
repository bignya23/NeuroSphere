...
const Dashboard = () => {
  ...
  const user = JSON.parse(localStorage.getItem("user")) || { name: "John Doe", profilePic: "https://randomuser.me/api/portraits/men/45.jpg" };
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <button className="absolute top-4 left-4 md:hidden z-50 bg-blue-500 text-white p-2 rounded-md" onClick={() => setIsSidebarOpen(true)}>
        <FiMenu size={24} />
      </button>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-white shadow-lg transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform md:relative md:translate-x-0 flex flex-col`}>
        ...
        {/* Close Button (Mobile) */}
        <button className="absolute top-4 right-4 md:hidden text-gray-600" onClick={() => setIsSidebarOpen(false)}>
          <FiX size={24} />
        </button>
        ...
        <div className="mt-auto p-6">
          <button onClick={handleLogout} className="flex items-center justify-center p-4 text-lg text-red-500 bg-red-100 hover:bg-red-500 hover:text-white rounded-lg mx-4 w-full">
